...
public interface JobRepository extends JpaRepository<Job, String> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Job j SET j.lockedBy = :nodeId, j.lockedAt = :now, j.status = com.scheduler.core.JobStatus.RUNNING " +
           "WHERE j.id = :jobId AND j.lockedBy IS NULL AND j.status = com.scheduler.core.JobStatus.PENDING")
